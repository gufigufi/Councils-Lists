...
import { detectCountryByState } from "../utils/location.js";
...
export const createCouncil = async (req, res) => {
  try {
    const { purchaseDate, endDate, name, email, address, zipCode, state } =
      req.body;
    ...
    // Автоматически определяем страну по штату/провинции
    const country = detectCountryByState(state);
    console.log(`🌍 Определена страна: ${country} для штата ${state}`);

    // Geocode the address
    const fullAddress = `${address}, ${zipCode}, ${state}`;
    const coordinates = await geocodeAddress(fullAddress, country);
    ...
    console.log(`✅ Геокодинг успешен: ${coordinates.displayName}`);
...
export const updateCouncil = async (req, res) => {
  try {
    ...
    let country = existingCouncil.country;

    if (
      address !== existingCouncil.address ||
      zipCode !== existingCouncil.zipCode ||
      state !== existingCouncil.state
    ) {
      // Автоматически определяем страну по новому штату
      country = detectCountryByState(state);
      console.log(`🌍 Определена страна: ${country} для штата ${state}`);
      ...
      if (coordinates) {
        latitude = coordinates.lat;
        longitude = coordinates.lon;
        console.log(`✅ Обновлены координаты: ${coordinates.displayName}`);
      }
    }
...
export const findNearestCouncils = async (req, res) => {
  try {
    ...
    // Пробуем геокодить сначала как USA, потом как Canada
    let searchCoords = await geocodeAddress(address, "USA");

    if (!searchCoords || searchCoords.lat < 24 || searchCoords.lat > 72) {
      // Если не нашли в США или координаты вне Северной Америки, пробуем Канаду
      searchCoords = await geocodeAddress(address, "Canada");
    }
    ...
    console.log(`🔍 Поиск от координат: ${searchCoords.displayName}`);
    ...
    const councilsWithDistance = allCouncils
      .map((council) => ({
        ...council,
